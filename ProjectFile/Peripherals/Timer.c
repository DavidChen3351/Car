...
Timer_motoHandle motoHandles[MOTO_NUM] = {
    {&leftMotoPWMHandle, &leftMotoEncoderHandle, leftMotoForCH, leftMotoBackCH, 0},
    {&rightMotoPWMHandle, &rightMotoEncoderHandle, rightMotoForCH, rightMotoBackCH, 0}
};

int32_t calRawDiff(uint16_t current, uint16_t prev);
...
/*
*return the counter difference between current and previous, and update previous counter
*/
...
/*
*return meter gain from last call for moto by index
*/
inline float Timer_GetMotoGain(motoIndex index)
{
    return (float)Timer_GetMotoDiff(index) / (float)COUNTER_PER_ROUTE * WHEEL_CIRCUMFERENCE;
}

/*
*return the counter difference between current and previous, and update previous counter
*/
inline int32_t Timer_GetMotoDiff(motoIndex index)
{
    Timer_motoHandle* handle = &motoHandles[index];
    uint16_t currentCounter = __HAL_TIM_GetCounter(handle->motoEncoder_htim);


    int32_t counterDiff = calRawDiff(currentCounter, handle->previousCounter);
    handle->previousCounter = currentCounter;
    return counterDiff;
}
